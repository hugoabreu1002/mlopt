...
class AttentionBlock(keras.Model):
    def __init__(self, name='AttentionBlock', num_heads=2, head_size=128, ff_dim=None, dropout=0, **kwargs):
        ...
        self.attention = MultiHeadAttention(num_heads=num_heads, key_dim=num_heads, dropout=dropout)
...
class AGTransformerKeras():
    def __init__(self,X_train, y_train, X_test, y_test, num_generations, size_population, prob_mut=0.2, epochs_per_individual=200):
    ...
    def _gen_population(self):
        """
            ...
            individual_template = [head_size, num_heads, num_layers, Transformer_Object, Fitness]
        """
        sizepop=self._size_population
        population = [['']]*sizepop

        for i in range(0, sizepop):
            population[i] = [random.randint(100,400),
             random.randint(1, 10),
             random.randint(1, 5), 
             'Transformer-object', 
             10]
    ...
    def _set_fitness_and_sort(self, population, start_set_fit):
    ...
    def _cruzamento(self, population):
    ...
    def _new_gen(self, population, num_gen):
        population = self._cruzamento(population)
        population = self._mutation(population)
        start_set_fit = int(self._size_population*num_gen/(2*self._num_generations))
        population = self._set_fitness_and_sort(population, start_set_fit)
        return population
    ...
    def search(self):
        ...
        population = self._set_fitness_and_sort(population, 0)
        ...
        for ng in tqdm(range(0, self._num_generations)):
            population = self._new_gen(population, ng)
